...
layout0 = BoxLayoutX(orientation="vertical", padding=0)  # главный слой
layout0.set_bgcolor(.1, .1, 1, .2)
...
layout2 = BoxLayout(orientation="horizontal", padding=0, size_hint=(1, .4))
...
layout_history = GridLayout(cols=1, spacing=1, size_hint_y=None)
root = ScrollView(size_hint=(1, 1), size=(Window.width, Window.height))
root.add_widget(layout)
root_history = ScrollView(size_hint=(1, 1), size=(Window.width, Window.height))
root_history.add_widget(layout_history)
...
solution = Label(text='0.0', font_size=32, size_hint=(1, .4), color=[1, 0, 1, 1])
summer = Button(text='0.0', bold= True, markup=True, font_size=120, size_hint=(1, 1), color=[1, 1, 0, 1])
...
toch = Button(text='.', font_size=62, markup=True)
lay0.add_widget(clear_layout1)
lay0.add_widget(history_layout1)
...
layout_history.bind(minimum_height=layout_history.setter('height'))
...
def file_clear():
    ...
    with open('kalkulator.txt') as f:              
                ...
                for j in range(0, len(sp_all)):
                    s = sp_all[j].split()
                    if data.strip() == s[0].strip():
                        break
                    else:
                        with open('kalkulator.txt', 'w') as f1:   
                            f1.write(data+' 00:00:'+' 0.0\n')
                
file_clear()

def remove(t):
    global cl_lay1, activ_button1, activ_button2
    if activ_button2 == False:
        pass
    else:
        if t == True:
            layout0.remove_widget(layout1)
            clear_layout1.text = '[b]показать кнопки[/b]'
            history_layout1.text = ''
...
def remove_all(t):
    global cl_all, activ_button1, activ_button2
    k = 0
    if activ_button1 == False:
        pass
    else:
        if t == True:
            layout0.remove_widget(layout1)
            ...
            for j in range(-1, -len(sp_all), -1):
                ...
                if data.strip() == s[0].strip():
                    k += 1
                    setka_history(k, k + 1, s[0] + ' ' + s[1], s[2])
...
def setka_history(i1, i2, data, sum):
    for i in range(i1, i2):
        ...
        for j in range(2):
            entry[i, j] = LabelX(halign="center", font_size=40, size_hint=(None, None), height=60, width=400,
                                 color=[0, 0, 0, 1])
    ...
    for i in range(i1, i2):
        ...
        for j in range(6):
            if j == 5:
                ...
                entry[i, j] = LabelX(halign="center", font_size=50, size_hint=(None, None), height=60, width=50,
                                     color=[0, 0, 0, 1])
                entry[i, j].set_bgcolor(1, 1, 1, 1)
...
def clear_calk():
    ...
    if pust_chek == True:
        ...
        popupWindow = Popup(title="Вы уверены, что хотите удалить? ", size_hint=(None, None), size=(400, 220),
                            pos_hint={'center_x': 300.0 / Window.width, 'center_y': 600.0 / Window.height})
...
history_layout1.on_press = lambda: remove_all(cl_all)
runTouchApp(layout0)