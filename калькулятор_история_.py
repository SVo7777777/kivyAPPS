...
#lay0.add_widget(history_layout2)
    ...
    for i in range(i1, i2):
        ...
        for j in range(6):
            if j == 5:
                ...
                entry[i, j] = LabelX(halign="center", font_size=50, size_hint=(None, None), height=60, width=50, color=[0, 0 ,0, 1])
                entry[i, j].set_bgcolor(1,1,1,1)
...
def skidka():
    ...
    if pust_chek == True:
        ...
        popupWindow = Popup(title="скидка", size_hint=(None,None),size=(650,200), pos_hint={'x': 40.0 / Window.width, 'y': 550.0 / Window.height})
