...
class MainApp(App):
    def build(self):
        ...
        layout_itog = GridLayout(cols=1, spacing=0, size_hint=(.38, 1))
        ...
        lab1 = LabelX(halign="center", text='Выберите год и месяц! Введите часы!\n Чтобы добавить, жмите на "+"! Обновите! ', font_size=35, size_hint=(1, .3), color=[0, 0 ,0, 1])
        ...
        def setka(i1, i2):
            ...
            for i in range(32):
                h_layout = BoxLayout(height=794,orientation="vertical", size_hint=(None, None))
                for j in range(i1, i2):
                    num[i,j]= TextInput(halign="center", cursor_color=[0, 0, 1, 1], font_size=40, size_hint=(1, 1),  width=60, multiline=False, text='')    
            ...
            for i in range(i2):                  
                  itog[i] = LabelX(halign="right", height=61, width= 130, font_size=35, size_hint=(None, None), color=[1, 0 ,0, 1])
