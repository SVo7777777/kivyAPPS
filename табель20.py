            ...
            for i in range(i1, i2):
                ...
                for j in range(4):
                    if j == 0 or  j == 2 or j == 3:
                        ...
                        self.entry[i, j] = TextInput(halign="center", cursor_color=[0, 0, 1, 1], font_size=45, size_hint=(1, 1),  width=50, multiline=False)
            ...
            if  self.crsp == False:
                with open('tabel_sotrudnikov.txt', 'r') as f:              
                        if sotrudnik in f.read():
                            ...
                            self.manager.get_screen('tabel').entry[r, c-1].text = sotrubnik
                            with open('tabel_sotrudnikov.txt', 'a+') as k:
...
class  TabelScreen(Screen):
    def __init__(self, name, col2, **kwargs):
        ...
        to_spisok = Button(text='перейти  к\nсозданию списка', markup=True, font_size=35, size_hint=(1, 1), color=[1, 1, 0, 1])
        ...
        to_spisok.on_press = lambda: to_spisok_screen()     
    ...
    def setka(self, i1, i2):
            ...
            for i in range(32):
                h_layout = BoxLayout(height=45*il2, orientation="vertical", size_hint=(None, None))#height=794,
            ...
            for i in range(i2):                  
                  itog[i] = LabelX(halign="right", height=45, width= 130, font_size=35, size_hint=(1, None), color=[1, 0 ,0, 1])
